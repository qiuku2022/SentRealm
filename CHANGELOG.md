...
- **结果行定位原文**：点击右侧断句结果行，中间原文滚动到对应片段并两次脉冲高亮

### Changed

- **侧栏布局**：移除侧栏顶部的应用图标与名称，折叠按钮并入说明行；应用名仍保留在窗口顶栏
- **界面线条**：主界面仅保留正文编辑区外框与左右侧栏竖向分隔线；移除顶栏、分区标题、统计区等布局分隔线；按钮边框保持不变
- **界面可读性**：辅助文字（统计栏、结果行号、快捷键提示等）字号由 10–11.5px 提升至 12–12.5px，并加粗（字重 500）
- **字体统一**：全界面统一为微软雅黑（`Microsoft YaHei UI` / `Microsoft YaHei`），移除 Consolas 等等宽字体；并显式覆盖 `input` / `textarea` / `button` 等控件的浏览器默认字体

---

## [0.5.2] - 2026-08-10 — M3 内部构建

### Changed

- **品牌图标**：小尺寸圆角加强（12px@48px 等效），修复桌面快捷方式仍显示为方角的问题
...
- **品牌图标**：按 Windows 11 窗口圆角规范（8px @ 96 DPI）重新裁切圆角并生成全套桌面 / 安装包 / 侧栏图标
...
- **自然边界规则断句**：标点从“全部原位换行”改为硬边界、超长候选与仅删除三级处理；规则层改为全局选点并增加短行回并，高风险单字断点降级，宁可标记局部超长也不制造语法碎片（ADR-012）
...
- **出厂断句词表 v3**：基于五篇真实财经口播稿补充通用高频谓语起点（`可能`、`需要`、`可以`、`仍然`、`能够`、`不能`、`真正`、`通常`、`容易`、`提供`），并保护常见否定/情态组合；新安装与「恢复默认」生效，已有 SQLite 配置不强制覆盖
...
| `0.5.1` | M3 内部构建 | 品牌图标圆角裁切与 sidecar 启动修复 |
| `0.5.2` | M3 内部构建 | 小尺寸图标圆角加强，修复桌面方角显示 |
| `Unreleased` | — | 后续未发布变更 |
