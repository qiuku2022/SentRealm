...
**M1 / M2 已完成。** M3 **内部可用（NSIS）**：Windows 安装包可本地打出（PyInstaller onedir sidecar + Tauri NSIS，见 [packaging.md](./packaging.md)）。**作者工作环境以 NSIS 日常使用约 1 个月**，主流程与断句质量作者侧满意（档位 1 内部验收，2026-09-15）。仍阻塞正式 `1.0.0`：严格干净机冒烟、代码签名、正式公开发布。用户手册安装/分发表述已与内部 NSIS 对齐。ADR-009 物理拆包仍延后。
...
8. **安装包**：脚本与 sidecar 资源路径在仓库内；作者侧 NSIS 日常约 1 个月已覆盖主流程；本核对日未在严格干净机重跑 packaging 清单

## 本机执行面

下一优先：**冲正式 `1.0.0`**（严格干净机冒烟、代码签名、正式公开发布）。
