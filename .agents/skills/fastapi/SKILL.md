---
name: fastapi
description: FastAPI best practices and conventions. Use when working with FastAPI APIs, Pydantic models, dependencies, and streaming responses including Server-Sent Events (SSE) in apps/gui/api. Do not serve the frontend from FastAPI in this repo (Tauri WebView). Keeps FastAPI code clean and up to date with the latest features and patterns.
---
...
> **SentRealm**：本拷贝来自锁定的 `fastapi==0.139.2`。改 `apps/gui/api` 前先读同目录 [`SENTREALM.md`](./SENTREALM.md)。**禁止** `app.frontend()` / SQLModel；开发入口不是 `fastapi dev`。
